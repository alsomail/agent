...
- 硬上限 800 行
- 超过硬上限必须拆分模块
- 当文件接近 400 行，或出现多个独立变化原因时，应主动评估拆分，不要等到 800 行才处理。
- 拆分依据是**职责边界和变化原因**，不是机械按行数切割：
  - React Hook：按状态域/副作用拆分，例如 `session`、`streaming`、`modelConfig`、`messageTransform`。
  - React 组件：按可独立理解和复用的 UI 区块拆分，容器负责编排，子组件负责展示或局部交互。
  - 服务端路由：按资源或子能力拆分；校验、存储访问、Provider 探测等可复用逻辑下沉到 helper/service。
  - Store/Repository：CRUD、查询组装、领域转换可以分开；数据库行到协议类型的映射应集中在边界层。
  - 文档：超过单一阅读目标时拆分；架构决策、协议细节、实现步骤、验证清单分别放到对应文档，并用索引互链。
- 拆分后的每个文件应能用一句话说明职责；如果一句话里出现多个“和/以及/同时”，通常还可以继续拆。
- 拆分时保持唯一性原则：移动内容到新的唯一来源，原位置只保留链接或简短索引，不复制同一细节。
